...
class CuentaPorCobrarSerializer(serializers.ModelSerializer):
    ...
    estudiante_matricula = serializers.CharField(source='estudiante.matricula')
    seccion_nombre = serializers.CharField(source='seccion.nombre')
    facturas = serializers.SerializerMethodField()

    class Meta:
        model = CuentaPorCobrar
        fields = ['id', 'estudiante_nombre', 'estudiante_matricula',  'estudiante_apellidos','seccion_nombre', 'monto', 'fecha_factura', 'estado', 'facturas']
